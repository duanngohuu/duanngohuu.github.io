(()=>{
  ...
  let observer,timer,frame,fileId='',objectUrl='',objectFileId='',accessToken='',tokenExpiresAt=0,loading=null;
  ...
  function clearBlob(){if(objectUrl){URL.revokeObjectURL(objectUrl);objectUrl='';objectFileId='';}}
  function detect(){
    frame=currentFrame();if(!frame)return false;
    const detected=frame.dataset.driveFileId||driveId(frame.getAttribute('src')||frame.src);
    if(!detected)return false;
    if(fileId&&detected!==fileId)clearBlob();
    fileId=detected;frame.dataset.driveFileId=fileId;frame.dataset.previewPdfSrc=preview(fileId,1);frame.setAttribute('title','PDF sách trên Google Drive');return true;
  }
  function waitForGIS(){
    return new Promise((resolve,reject)=>{
      const started=Date.now();
      const check=()=>{if(window.google?.accounts?.oauth2)return resolve();if(Date.now()-started>10000)return reject(new Error('Google Identity Services chưa tải được.'));setTimeout(check,100);};check();
    });
  }
  async function requestToken(){
    if(accessToken&&Date.now()<tokenExpiresAt-60000)return accessToken;
    const id=clientId();if(!id){window.BJT_AUDIO?.openAuthSheet?.();throw new Error('Chưa cấu hình OAuth Client ID để điều khiển trang PDF.');}
    await waitForGIS();
    return new Promise((resolve,reject)=>{
      const tokenClient=google.accounts.oauth2.initTokenClient({client_id:id,scope,callback:r=>{if(r.error){reject(new Error(r.error_description||r.error));return;}accessToken=r.access_token;tokenExpiresAt=Date.now()+(+(r.expires_in||3600))*1000;resolve(accessToken);},error_callback:e=>reject(new Error(e?.message||'Không đăng nhập được Google Drive.'))});
  ...
  async function ensureNative({page=1,interactive=false,force=false}={}){
    ...
    if(objectUrl&&objectFileId===fileId&&!force){setNativePage(page);return true;}
    ...
    loading=(async()=>{
      try{
        ...
        const res=await fetch(`https://www.googleapis.com/drive/v3/files/${encodeURIComponent(requestedFileId)}?alt=media`,{headers:{Authorization:`Bearer ${token}`}});
        ...
        if(requestedFileId!==fileId)return false;
        clearBlob();objectUrl=URL.createObjectURL(blob);objectFileId=requestedFileId;setNativePage(page);return true;
  ...
  function setPage(page,{interactive=false}={}){
    if(detect()&&objectUrl&&objectFileId===fileId)return Promise.resolve(setNativePage(page));
    setFallback(page);if(!interactive)return Promise.resolve(false);return ensureNative({page,interactive:true}).catch(()=>false);
  }
  function reload(page=1,{interactive=false}={}){
    if(detect()&&objectUrl&&objectFileId===fileId){const src=nativeSrc(page);frame.setAttribute('src','about:blank');setTimeout(()=>frame?.setAttribute('src',src),80);return Promise.resolve(true);}
  ...
  function enhance(){if(!detect())return;const page=window.BJT_PDF?.getPage?.()||1;if(objectUrl&&objectFileId===fileId)setNativePage(page);else setFallback(page);}
  ...
  window.addEventListener('load',enhance);window.addEventListener('beforeunload',clearBlob);
