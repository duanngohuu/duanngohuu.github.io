(()=>{
  ...
  function changeLesson(id){
    ...
    state.lessonId=id;state.questionId='';
  ...
  function ensureDock(){
    ...
    dock.id='mobileFloatingDock';
    dock.className='mobile-floating-dock';
    dock.setAttribute('aria-label','Điều khiển nổi khi học BJT');
  ...
  function renderDock(){
    ...
    const d=ensureDock();
    const all=state.lessons.filter(x=>x.book_id===state.bookId).sort(lessonSort);
    const sessions=[...new Set(all.map(x=>x.part_title||'Toàn bộ'))];
    const session=lesson.part_title||'Toàn bộ';
    const lessonList=all.filter(x=>(x.part_title||'Toàn bộ')===session);
    const media=window.BJT_AUDIO?.getList?.()||[];
    const currentMedia=window.BJT_AUDIO?.getCurrent?.();
    const hasMemo=Boolean(localStorage.getItem(`bjtMemoCurrent:${lesson.lesson_id}`)?.trim());
    d.innerHTML=`<div class="float-audio-row"><button id="floatPlay" class="float-play" type="button">▶</button><select id="floatTrack" aria-label="Chọn track">${media.length?media.map((m,i)=>`<option value="${esc(m.media_id)}" ${m.media_id===currentMedia?.media_id?'selected':''}>${String(i+1).padStart(2,'0')} · ${esc(m.track_label||m.file_name)}</option>`).join(''):'<option>Chưa có audio</option>'}</select><button class="float-step" data-step="-1" type="button">‹</button><button class="float-step" data-step="1" type="button">›</button></div><div class="float-select-row"><select id="floatSession" aria-label="Chọn session">${sessions.map(s=>`<option value="${esc(s)}" ${s===session?'selected':''}>${esc(s)}</option>`).join('')}</select><select id="floatLesson" aria-label="Chọn bài">${lessonList.map((l,i)=>`<option value="${esc(l.lesson_id)}" ${l.lesson_id===lesson.lesson_id?'selected':''}>${String(i+1).padStart(2,'0')} · ${esc(l.section_title)}</option>`).join('')}</select></div><div class="float-jump-row"><button data-jump="contentPane" class="active">Bài</button><button data-jump="pdfPane">PDF</button><button data-jump="memoPanel" class="${hasMemo?'has-note':''}">Memo</button><button data-jump="lessonGroups">Mục lục</button></div>`;
    ...
    d.querySelector('#floatTrack').onchange=e=>window.BJT_AUDIO?.selectMedia?.(e.target.value,{autoplay:true});
    ...
    const update=()=>{
      ...
      const show=a.bottom<8&&workspace&&workspace.bottom>100;
      d.classList.toggle('visible',Boolean(show));
      const line=(show?d.getBoundingClientRect().height:0)+14;
      let active='contentPane';
    ...
    window.onscroll=update;window.onresize=update;update();
  }
  ...
  window.addEventListener('bjt-audio-state',e=>{
    ...
    const track=dock.querySelector('#floatTrack');if(track&&e.detail.media)track.value=e.detail.media.media_id;
  });
  window.addEventListener('DOMContentLoaded',()=>{
    const root=document.getElementById('lessonDetail');
    if(root){observer=new MutationObserver(()=>{clearTimeout(timer);timer=setTimeout(renderDock,40);});observer.observe(root,{childList:true,subtree:true});}
  });
