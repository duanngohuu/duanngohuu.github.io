(()=>{
  ...
  function setPdfPage(book,lesson,page,{force=false}={}){
    ...
    localStorage.setItem(pageKey(lesson.lesson_id),String(currentPage));localStorage.setItem(lastBookPageKey(book.book_id),String(currentPage));localStorage.setItem(lastLessonKey(book.book_id),lesson.lesson_id);
  ...
  function ensurePicker(){
    ...
    picker.onclick=e=>{if(e.target===picker)closePicker();};picker.querySelector('#pdfPickerClose').onclick=closePicker;
    ...
    picker.querySelector('#pdfZoomOutSheet').onclick=()=>setZoom(currentZoom-.2);picker.querySelector('#pdfZoomResetSheet').onclick=()=>setZoom(1);picker.querySelector('#pdfZoomInSheet').onclick=()=>setZoom(currentZoom+.2);
    picker.querySelector('#pdfFullscreenSheet').onclick=toggleFullscreen;picker.querySelector('#pdfCaptureSheet').onclick=captureScreen;
  ...
  async function captureScreen(){
    closePicker();
    if(!navigator.mediaDevices?.getDisplayMedia){alert('Trình duyệt này không cho website tự chụp màn hình. Trên iPhone: bấm đồng thời nút nguồn + tăng âm lượng. Trên máy tính, mở bằng Chrome/Safari mới để dùng nút Chụp.');return;}
    let stream;
    try{
      ...
      const blob=await new Promise(r=>canvas.toBlob(r,'image/png',1));const file=new File([blob],`BJT-${currentLesson?.lesson_id||'PDF'}-page-${currentPage}.png`,{type:'image/png'});
      if(navigator.share&&navigator.canShare?.({files:[file]}))await navigator.share({files:[file],title:`BJT trang ${currentPage}`});
      else{const a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=file.name;a.click();setTimeout(()=>URL.revokeObjectURL(a.href),2000);}toast('Đã chụp màn hình');
  ...
  function enhancePdfState(){
    ...
    document.getElementById('pdfPrevPage').onclick=()=>setPdfPage(book,lesson,currentPage-1,{force:true});document.getElementById('pdfNextPage').onclick=()=>setPdfPage(book,lesson,currentPage+1,{force:true});document.getElementById('pdfOpenPagePicker').onclick=openPicker;
    document.getElementById('pdfZoomOut').onclick=()=>setZoom(currentZoom-.2);document.getElementById('pdfZoomIn').onclick=()=>setZoom(currentZoom+.2);document.getElementById('pdfZoomLabel').onclick=()=>setZoom(1);
    document.getElementById('pdfFullscreen').onclick=toggleFullscreen;document.getElementById('pdfCapture').onclick=captureScreen;
    frame.addEventListener('load',updateLabels);
    const onPageShow=updateLabels;window.addEventListener('pageshow',onPageShow);removeListeners=()=>window.removeEventListener('pageshow',onPageShow);
