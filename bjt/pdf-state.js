(()=>{
  ...
  const pageKey=id=>`bjtPdfPageV2:${id}`;
  const zoomKey=id=>`bjtPdfZoom:${id}`;
  const lastLessonKey=id=>`bjtPdfLastLessonV2:${id}`;
  const lastBookPageKey=id=>`bjtPdfLastPageV2:${id}`;
  ...
  function storedPage(book,lesson){
    ...
    if(!Number.isFinite(own)||own<start(lesson)||own>end(lesson))return start(lesson);
    return clampPage(book,own);
  }
  ...
  function basePdfSrc(frame){if(frame.dataset.basePdfSrc)return frame.dataset.basePdfSrc;const src=String(frame.getAttribute('src')||frame.src||'').split('#')[0];frame.dataset.basePdfSrc=src;return src;}
  ...
  function updateLabels(){const input=document.getElementById('pdfPageInput');if(input&&document.activeElement!==input)input.value=currentPage;const button=document.getElementById('pdfOpenPagePicker');if(button)button.textContent=`Trang ${currentPage}`;const zoom=document.getElementById('pdfZoomLabel');if(zoom)zoom.textContent=`${Math.round(currentZoom*100)}%`;const status=document.getElementById('pdfLoadStatus');if(status&&currentLesson)status.textContent=`Trang ${currentPage} · bài ${start(currentLesson)}–${end(currentLesson)}`;syncPicker();emit();}
  function setPdfPage(book,lesson,page,{force=false}={}){const frame=document.getElementById('pdfFrame');if(!frame)return;currentBook=book;currentLesson=lesson;currentPage=clampPage(book,page);localStorage.setItem(pageKey(lesson.lesson_id),String(currentPage));localStorage.setItem(lastBookPageKey(book.book_id),String(currentPage));localStorage.setItem(lastLessonKey(book.book_id),lesson.lesson_id);const base=basePdfSrc(frame);const sep=base.includes('?')?'&':'?';const next=`${base}${sep}bjtPage=${currentPage}&t=${Date.now()}#page=${currentPage}&zoom=page-width`;if(force||String(frame.getAttribute('src')||'')!==next)frame.setAttribute('src',next);updateLabels();}
  function setZoom(value){const frame=document.getElementById('pdfFrame'),lesson=currentLesson;if(!frame||!lesson)return;const {viewport,stage}=ensureViewport(frame),old=currentZoom,next=clampZoom(value);const centerX=(viewport.scrollLeft+viewport.clientWidth/2)/old,centerY=(viewport.scrollTop+viewport.clientHeight/2)/old;currentZoom=next;localStorage.setItem(zoomKey(lesson.lesson_id),String(next));stage.style.transform=`scale(${next})`;stage.style.width='100%';stage.style.minHeight='100%';requestAnimationFrame(()=>{viewport.scrollLeft=Math.max(0,centerX*next-viewport.clientWidth/2);viewport.scrollTop=Math.max(0,centerY*next-viewport.clientHeight/2);});updateLabels();}
  ...
  function ensurePicker(){
    ...
    picker.querySelector('#pdfStartPage').onclick=()=>setPdfPage(currentBook,currentLesson,start(currentLesson),{force:true});picker.querySelector('#pdfEndPage').onclick=()=>setPdfPage(currentBook,currentLesson,end(currentLesson),{force:true});
  ...
  function quickPages(){if(!currentBook||!currentLesson)return[];const max=maxPage(currentBook),values=new Set([start(currentLesson),end(currentLesson),currentPage-5,currentPage-1,currentPage,currentPage+1,currentPage+5]);for(let p=start(currentLesson);p<=end(currentLesson);p+=10)values.add(p);return [...values].map(p=>clampPage(currentBook,p)).sort((a,b)=>a-b);}
  function syncPicker(){if(!picker||picker.hidden||!currentBook||!currentLesson)return;picker.querySelector('#pdfPickerMeta').textContent=`Trang ${currentPage}/${maxPage(currentBook)} · bài ${start(currentLesson)}–${end(currentLesson)} · zoom ${Math.round(currentZoom*100)}%`;const input=picker.querySelector('#pdfPickerInput');if(document.activeElement!==input)input.value=currentPage;input.max=maxPage(currentBook);const range=picker.querySelector('#pdfPageRange');range.min=start(currentLesson);range.max=end(currentLesson);range.value=currentPage;const chips=picker.querySelector('#pdfPageChips');chips.innerHTML=quickPages().map(p=>`<button type="button" data-page="${p}" class="${p===currentPage?'active':''}">${p}</button>`).join('');chips.querySelectorAll('[data-page]').forEach(b=>b.onclick=()=>{setPdfPage(currentBook,currentLesson,b.dataset.page,{force:true});closePicker();});}
  ...
  function enhancePdfState(){
    ...
    setPdfPage(book,lesson,currentPage,{force:true});setZoom(currentZoom);
  }
