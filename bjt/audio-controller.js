(()=>{
  ...
  let mediaList=[],currentIndex=0,currentLessonId='',observer,timer,picker,scriptSheet;
  ...
  function emit(){window.dispatchEvent(new CustomEvent('bjt-audio-state',{detail:{lessonId:currentLessonId,index:currentIndex,total:mediaList.length,media:currentMedia(),playing:false,secure:true,preview:true}}));}
  ...
  function fillScript(){
    ...
    s.querySelector('#scriptMeta').textContent=m?`${m.track_label||m.file_name}${q?` · PDF trang ${q.pdf_page}`:''}`:'';
    const host=s.querySelector('#scriptContent');
    if(!q){host.innerHTML='<div class="script-empty">Track này chưa được map với một câu nghe.</div>';return;}
  ...
  function renderMedia(index){
    ...
    if(host)host.innerHTML=`<iframe id="bjtAudioFrame" src="${esc(preview(m.file_url))}" allow="autoplay; encrypted-media" referrerpolicy="strict-origin-when-cross-origin" title="Audio Google Drive"></iframe><div class="audio-secure-note">Dùng trực tiếp tài khoản Google đang đăng nhập trong trình duyệt. Chỉ email đã được share mới nghe được.</div>`;
    ...
    setStatus(`${m.track_label||m.file_name} · ${currentIndex+1}/${mediaList.length} · Drive Preview`);
    updateScriptButton();syncPicker();emit();
  }
  function selectMedia(id,{close=true}={}){const i=mediaList.findIndex(m=>m.media_id===id);if(i>=0){renderMedia(i);if(close)closePicker();}}
  function step(delta){if(!mediaList.length)return;const next=Math.min(mediaList.length-1,Math.max(0,currentIndex+delta));if(next!==currentIndex)renderMedia(next);}
  function toggle(){document.getElementById('studyAudioBar')?.scrollIntoView({behavior:'smooth',block:'start'});setTimeout(()=>document.getElementById('bjtAudioFrame')?.focus(),350);}
  function setMode(mode){
    ...
    if(mode==='question')renderMedia(requestedIndex(mediaList,lesson()));
    if(mode==='full')renderMedia(0);
    syncPicker();
  }
  ...
  function renderPickerList(){
    ...
    host.querySelectorAll('[data-track-id]').forEach(b=>b.onclick=()=>selectMedia(b.dataset.trackId,{close:true}));
  }
  ...
  function enhance(){
    ...
    bar.innerHTML=`<div class="audio-controller-head"><div class="audio-info"><strong>♫ Audio luyện nghe <span class="audio-auth-badge">Google account</span></strong><small id="audioStatus">${mediaList.length?`${mediaList.length} track trong bài`:'Bài này chưa có track'}</small></div><div class="audio-mode"><button type="button" data-audio-mode="question" class="${mode==='question'?'active':''}">Theo câu</button><button type="button" data-audio-mode="full" class="${mode==='full'?'active':''}">Từ đầu bài</button></div></div><div class="audio-controller-row"><button id="audioPrev" class="audio-icon" type="button">‹</button><button id="audioOpenPicker" class="audio-track-button" type="button">${m?`<span>${trackNo(m,currentIndex)}</span><b>${esc(m.track_label||m.file_name)}</b><i>${currentIndex+1}/${mediaList.length}</i>`:'<b>Chưa có audio</b>'}</button><button id="audioNext" class="audio-icon" type="button">›</button></div><div class="audio-preview-actions"><button data-script-open class="audio-script-btn" type="button">Script</button><span>Play/pause và thời gian dùng trực tiếp trong player Google Drive.</span></div><div id="audioPlayerHost" class="audio-player-host"></div>`;
    bar.querySelectorAll('[data-audio-mode]').forEach(b=>b.onclick=()=>setMode(b.dataset.audioMode));
    document.getElementById('audioOpenPicker').onclick=openPicker;document.getElementById('audioPrev').onclick=()=>step(-1);document.getElementById('audioNext').onclick=()=>step(1);bar.querySelector('[data-script-open]').onclick=openScript;
    if(mediaList.length)renderMedia(currentIndex);closePicker();closeScript();
  }
  ...
  window.BJT_AUDIO={toggle,step,selectMedia,setMode,openPicker,closePicker,openScript,closeScript,getList:()=>mediaList.slice(),getCurrent:currentMedia,getIndex:()=>currentIndex,enhance};
})();