(()=>{
  ...
  function emit(){
    ...
    window.dispatchEvent(new CustomEvent('bjt-audio-state',{detail:{lessonId:currentLessonId,index:currentIndex,total:mediaList.length,media:m,playing:false,secure:true,mode:localStorage.getItem(modeKey(currentLessonId))||'question'}}));
  }
  ...
  function renderMedia(index){
    ...
    setStatus(`${m.track_label||m.file_name} · ${currentIndex+1}/${mediaList.length} · Google account`);
    syncPicker();emit();
  }
  ...
  function step(delta){if(!mediaList.length)return;renderMedia((currentIndex+delta+mediaList.length)%mediaList.length);}
  function setMode(mode){
    ...
    const l=lesson();
    if(mode==='question')renderMedia(requestedIndex(mediaList,l));
  ...
  function ensurePicker(){
    ...
    picker.innerHTML=`<section class="audio-picker-sheet" role="dialog" aria-modal="true" aria-label="Chọn audio"><header><div><strong>Chọn track audio</strong><small id="audioPickerMeta"></small></div><button id="audioPickerClose" type="button">×</button></header><div class="audio-picker-tools"><label>⌕ <input id="audioPickerSearch" placeholder="Nhập 26, CD1-26, tên file…" inputmode="search"></label><div class="audio-picker-mode"><button type="button" data-picker-mode="question">Theo câu</button><button type="button" data-picker-mode="full">Từ đầu bài</button></div></div><div class="audio-picker-auth">🔒 Dùng phiên đăng nhập Google của trình duyệt. File vẫn để Restricted và chỉ email đã share mới truy cập được.</div><div id="audioPickerList" class="audio-picker-list"></div></section>`;
  ...
  function syncPicker(){
    ...
    picker.querySelector('#audioPickerMeta').textContent=`${mediaList.length} track · đang chọn ${currentIndex+1} · Drive Restricted`;
    picker.querySelectorAll('[data-picker-mode]').forEach(b=>b.classList.toggle('active',b.dataset.pickerMode===mode));
    renderPickerList();
  }
  function openPicker(){
    const p=ensurePicker();p.hidden=false;document.body.classList.add('picker-open');
  ...
  function enhance(){
    ...
    if(mediaList.length)renderMedia(currentIndex);
    closePicker();
  }
